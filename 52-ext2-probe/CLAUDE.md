...
**목표**: ext2 파일시스템의 온디스크 구조를 읽기 전용으로 파싱·검증한다. 슈퍼블록과 블록 그룹 디스크립터를 읽고, 루트 디렉토리(inode 2)의 엔트리를 나열해 커널 로그로 확인한다. FS 파싱 결과를 기존 VFS(`vfs_ops_t`)에 연결하는 일은 `53-vfs-ext2-read`로 미룬다 — 이번 단계는 온디스크 구조를 올바르게 읽어내는 것 자체가 목표다.

**이름이 "mount"가 아니라 "probe"인 이유**: 이 프로젝트에서 "마운트"는 `vfs_mount(prefix, ops)`로 VFS 네임스페이스의 한 경로에 백엔드를 등록하는 동작을 가리킨다(`boot/vfs.c`). 이번 단계는 그걸 하지 않는다 — `ext2_probe()`는 슈퍼블록/그룹 디스크립터/루트 inode를 읽어 이미지가 유효한 ext2인지 확인하고, 루트 디렉토리 파싱 로직을 커널 로그로 스모크테스트할 뿐 `vfs_mount()`를 호출하지 않는다. 실제로 ext2가 VFS 마운트 테이블에 등록되는 건 `53-vfs-ext2-read`다.
...
| `s_magic` | 0x38 | `0xEF53` — 이 값이 아니면 probe 중단 |
...
`kernel_main`이 `ata_init()` 직후(51의 raw sector dump 자리를 대체) `ext2_probe()` 하나만 호출한다. 이 함수는:
...
- `53-vfs-ext2-read`가 지금 확정한 `ext2_probe()`(및 그 안의 슈퍼블록/그룹/inode 파싱 로직)를 재사용 가능한 형태로 쪼개서 `vfs_ops_t.open`/`read`/`close`에 연결하고, 그 시점에 비로소 `vfs_mount()`로 ext2를 VFS 네임스페이스에 등록한다 — 지금은 `ext2_probe()`가 로그만 찍고 끝나지만, 다음 단계에서는 inode 번호로 파일을 찾아 열고 데이터 블록을 읽는 경로가 필요하다.
...
- 블록 그룹이 1개보다 많은 경우(더 큰 디스크 이미지)는 여전히 스코프 밖이다 — `ext2_probe()`는 그룹 0만 읽는다.
