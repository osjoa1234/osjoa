...
static void activate_thread(thread_t *t)
{
...
thread_t *thread_create_with_data(thread_fn_t fn, void *data)
{
    ...
    stack = (u8 *)kmalloc(THREAD_STACK_SIZE);
    if (!stack) { kfree(t); return 0; }

    sp = (u64 *)(stack + THREAD_STACK_SIZE);
...
void scheduler_tick(void)
{
    ...
    if (prev == &idle_task) {
        ...
        activate_thread(next);
        current = next;
        switch_context(&idle_task.rsp, next->rsp);
        return;
    }
