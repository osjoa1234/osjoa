...
static void thread_a(void)
{
    ...
    for (i = 0U; i < 3U; i++) {
        console_printf("thread A: tick %u\n", i);
        thread_yield();
    }
}
...
static void thread_b(void)
{
    ...
    for (i = 0U; i < 3U; i++) {
        console_printf("thread B: tick %u\n", i);
        thread_yield();
    }
}
...
void kernel_main(u32 magic, u32 phys_mbi)
{
    ...
    console_printf("Hello world -- long mode (64-bit), threads\n");
    ...
    console_printf("threads: 2 kernel threads created\n");
    ...
    console_set_color(0x0A);
    console_printf("long mode: paging+kheap+thread port complete (usermode in 40)\n");
