...
u32 paging_init(u32 mmap_vaddr, u32 mmap_length)
{
    ...
    {
        u32 cr3_phys = (u32)(u64)page_directory - KERNEL_OFFSET;
        kernel_pd_phys = cr3_phys;
        __asm__ volatile ("mov %0, %%cr3" : : "r"((u64)cr3_phys));
        return cr3_phys;
    }
}
...
{
    __asm__ volatile ("mov %0, %%cr3" : : "r"((u64)kernel_pd_phys));
}
...
{
    u32 *pd  = (u32 *)(pd_phys + KERNEL_OFFSET);
    u32 *pt0 = (u32 *)((pd[0] & ~0xFFFU) + KERNEL_OFFSET);
    u32  i;

    for (i = 0U; i < 1024U; i++) {
        if (pt0[i] & 0x01U) {
            page_free(pt0[i] & ~0xFFFU);
            pt0[i] = 0U;
        }
    }
    flush_tlb();
...
{
    u32 *src_pd   = (u32 *)(src_pd_phys + KERNEL_OFFSET);
    u32 *dst_pd   = (u32 *)(dst_pd_phys + KERNEL_OFFSET);
    u32 *src_pt0  = (u32 *)((src_pd[0] & ~0xFFFU) + KERNEL_OFFSET);
    u32 *dst_pt0  = (u32 *)((dst_pd[0] & ~0xFFFU) + KERNEL_OFFSET);
    u32  i, k;

    for (i = 0U; i < 1024U; i++) {
        if (!(src_pt0[i] & 0x01U)) continue;
        {
            u32  src_frame = src_pt0[i] & ~0xFFFU;
            u32  dst_frame = page_alloc();
            u8  *src_data  = (u8 *)(src_frame + KERNEL_OFFSET);
            u8  *dst_data  = (u8 *)(dst_frame + KERNEL_OFFSET);

            for (k = 0U; k < 0x1000U; k++) dst_data[k] = src_data[k];

            dst_pt0[i] = dst_frame | (src_pt0[i] & 0xFFFU);
        }
    }
    flush_tlb();
}
