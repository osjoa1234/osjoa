...
**목표**: 셸에 `>` 파일 리다이렉션을 추가한다. `48-pipe`에서 "쓰기 가능한 파일시스템이 없어서" 미뤘던 기능이고, `55-ext2-write`로 ext2 쓰기가, `59-exec-vfs-symlink`로 `PATH` 기반 실행이 갖춰졌으니 이제 막힘 없이 구현할 수 있다.
...
`user/init.c`의 `split_pipeline`이 `redirect_out[SHELL_STAGE_MAX]` 출력 배열을 하나 더 받는다. `|`와 같은 자리에서 토큰을 훑다가 `>`를 만나면 다음 토큰을 그 스테이지의 리다이렉트 대상 파일명으로 기록하고 두 토큰(`>`와 파일명) 모두 `stage_argv`에서 제외한다. 스테이지당 리다이렉트는 최대 1개(파이프라인 문법 단순화 — 리눅스 셸도 사실상 마지막 `>`만 유효하지만, 여기서는 아예 두 번째 `>`를 문법 오류로 처리한다).

메인 루프의 fork 자식 쪽 배선은 파이프 dup2와 정확히 같은 자리, 같은 패턴으로 넣었다 — pipe fd들을 정리한 **다음**, exec 하기 **직전**:
...
`sys_creat_trunc`는 기존 `sys_creat`(→`O_CREAT`) 바로 옆에 `O_CREAT | O_TRUNC`로 여는 새 래퍼다. 이 순서라면 파이프라인의 마지막 스테이지가 `>`까지 갖고 있는 경우(`cmd1 | cmd2 > file`)에도 파이프 stdout dup2가 먼저 걸린 뒤 리다이렉트가 그 위에 덮어써서 최종적으로 파일로 나간다 — 리눅스 셸의 실제 동작(마지막에 나열된 리다이렉션이 이긴다)과 같은 순서다.

부팅 시퀀스에 쓰는 고정 검증 코드용으로 `run_argv`(레디렉션 없음) 옆에 `run_argv_redirect(argv, path)`를 추가했다 — fork한 자식이 위와 똑같이 `sys_creat_trunc` + `dup2` + `close` 뒤 `exec_path`한다. `run_argv`가 매번 인터랙티브 프롬프트 없이 고정 `argv`로 명령 하나를 실행하듯, 이건 그 명령의 stdout을 고정 파일로 리다이렉트해 실행한다.
...
2. **인터랙티브 `>` 파싱**: `make run`(GUI) 또는 QEMU 모니터 `sendkey`로 프롬프트에 직접 `echo ... > /disk/....txt`를 타이핑해 `split_pipeline`의 `>` 토큰 처리 자체를 검증한다 — `48-pipe`가 `|`를 검증했던 것과 같은 방식이다(`run-nogui`는 키보드 입력이 없어 고정 시퀀스만 확인하고, 타이핑 경로는 자동화 밖이다). 이번 구현 중 QEMU unix-socket 모니터로 `sendkey`를 스크립트로 넣어 `echo interactive redirect > /disk/itest.txt` → `cat /disk/itest.txt`가 `interactive redirect`를 정확히 출력하는 것까지 직접 확인했다.
3. **회귀**: 51~59의 ext2 읽기/쓰기/`getdents`/심링크/PATH exec 검증 전부 이전과 동일하게 통과. `make run-nogui` 뒤 `e2fsck -f -n build/disk.img` 에러 없음.
...
| 파일 | 상태 | 설명 |
|------|------|------|
| `boot/vfs.h` | 수정 | `O_TRUNC 0x200U` 추가 |
...
| `user/init.c` | 수정 | `O_TRUNC`/`sys_creat_trunc` 추가; `split_pipeline`이 `redirect_out[]` 출력 인자를 받아 `>` 토큰 파싱; `run_argv_redirect()` 신규; 파이프라인 fork 루프에 리다이렉트 배선 추가; 부팅 시퀀스에 `echo`/트렁케이트 검증 블록 4개 추가 |
| `Makefile` | 수정 | `ROOTFSBINLINKS`에 `echo` 추가(busybox 멀티콜 심링크); `run-nogui` 타임아웃 15s→20s(검증 블록 4개 추가로 부팅 시퀀스가 더 길어짐) |
...
- **`>` 하나만 지원, `>>`(append)나 `<`(입력 리다이렉션) 없음**: `O_APPEND`(리눅스 값 `0x400U`)와 append 파싱은 이번에 다루지 않았다. 필요해지면 `redirect_out`과 나란히 `append_out` 플래그 배열을 추가하는 정도로 확장 가능하다.
