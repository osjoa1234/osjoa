...
- **경로 기반 `stat`(syscall 4) 미구현**: `boot/syscall.h`의 syscall 번호 열거에 fd 기반 `SYS_FSTAT=5`만 있고 경로 기반 `SYS_STAT=4`가 없다. `busybox sh`에 진입해 bare `cat` 같은 명령을 치면 ash가 `$PATH`(환경변수를 안 물려줘 busybox 내장 기본값 4개 디렉터리로 폴백)의 각 후보 경로를 `stat()`으로 확인하려다 4번 연속 `syscall: unimplemented rax=4` → `-1`(유저 공간에서 EPERM으로 해석)을 받고 셸 프로세스가 죽는다. `busybox cat /disk/hello.txt`처럼 실행 파일 이름을 바로 주면 `proc_exec`가 PATH 탐색 없이 initrd에서 직접 찾아 실행하므로 이 경로를 타지 않아 정상 동작한다. 나중에 고칠 때는 `vfs_open(path)` → (지금 `sys_fstat`이 채우는 것과 같은) `stat_t` 채우기 → `vfs_close`로 구현 자체는 간단하지만, 지금 `sys_fstat`이 `st_mode`/`st_size`를 항상 캐릭터 디바이스+0으로 고정 채우는 스텁이라 VFS에 "파일 타입(일반 파일/디렉터리)"과 "실제 크기"를 물어볼 방법이 아직 없다는 점, 그리고 지금 모든 syscall 실패가 원인 불문하고 `-1`(EPERM) 하나로 뭉개진다는 점(진짜 원인 구분이 필요하면 `ENOENT`/`ENOTDIR` 등 별도 errno 반환도 같이 다뤄야 함)까지 같이 묶일 가능성이 크다.
