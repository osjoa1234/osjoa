...
void kernel_main(u32 magic, u32 phys_mbi)
{
    ...
    {
        static u8 ata_buf[ATA_SECTOR_SIZE];
        static u8 ata_verify[ATA_SECTOR_SIZE];
        static const char ata_pattern[] = "ATA PIO 51 test byte-pattern OK ";
        u32 i;

        if (ata_read_sector(0U, ata_buf) == 0) {
            console_set_color(0x0FU);
            console_printf("ata: read lba=0 first16=");
            for (i = 0U; i < 16U; i++) {
                console_printf("%02X ", ata_buf[i]);
            }
            console_printf("\n");
        } else {
            console_set_color(0x0CU);
            console_printf("ata: read lba=0 failed\n");
        }

        for (i = 0U; i < ATA_SECTOR_SIZE; i++) {
            ata_buf[i] = (u8)ata_pattern[i % (sizeof(ata_pattern) - 1U)];
        }

        if (ata_write_sector(5U, ata_buf) == 0 && ata_read_sector(5U, ata_verify) == 0) {
            int ok = 1;

            for (i = 0U; i < ATA_SECTOR_SIZE; i++) {
                if (ata_buf[i] != ata_verify[i]) {
                    ok = 0;
                    break;
                }
            }
            console_set_color(ok ? 0x0AU : 0x0CU);
            console_printf("ata: write/read lba=5 verify=%s\n", ok ? "OK" : "FAIL");
        } else {
            console_set_color(0x0CU);
            console_printf("ata: write/read lba=5 failed\n");
        }
    }
