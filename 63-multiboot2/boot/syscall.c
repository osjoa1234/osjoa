...
static u32 sys_open(const char *path, u32 flags)
{
    process_t  *p = (process_t *)thread_current()->user_data;
    vfs_file_t *f;
    u32         i;

    f = vfs_open(path, flags);
    if (!f) return (u32)-1U;
...
static u32 sys_mkdir(const char *path)
{
    return (vfs_mkdir(path) == 0) ? 0U : (u32)-1U;
}

static u32 sys_unlink(const char *path)
{
    return (vfs_unlink(path) == 0) ? 0U : (u32)-1U;
}

static u32 sys_symlink(const char *target, const char *linkpath)
{
    return (vfs_symlink(target, linkpath) == 0) ? 0U : (u32)-1U;
}

static u64 sys_readlink(const char *path, char *buf, u32 bufsize)
{
    int n = vfs_readlink(path, buf, bufsize);
    return (n >= 0) ? (u64)(u32)n : (u64)-1LL;
}
...
static u32 sys_lstat(const char *path, stat_t *buf)
{
    u32 mode;
    u32 size;
    u8 *raw;
    ...
    if (vfs_lstat(path, &mode, &size) != 0) return (u32)-1U;
...
void syscall_dispatch(struct interrupt_frame *frame)
{
    ...
    case SYS_READ:
        ...
        break;
    case SYS_OPEN:
        frame->rax = (u64)(long long)(int)sys_open((const char *)frame->rdi, (u32)frame->rsi);
        break;
    case SYS_CLOSE:
    ...
    case SYS_STAT:
        ...
        break;
    case SYS_LSTAT:
        frame->rax = sys_lstat((const char *)frame->rdi, (stat_t *)frame->rsi);
        break;
    case SYS_FSTAT:
    ...
    case SYS_EXECVE:
        proc_exec((const char *)frame->rdi, (char *const *)frame->rsi, (char *const *)frame->rdx);
        frame->rax = (u64)-1;
        break;
    case SYS_EXIT:
        proc_thread_exit();
    ...
    case SYS_MKDIR:
        ...
        break;
    case SYS_UNLINK:
        frame->rax = sys_unlink((const char *)frame->rdi);
        break;
    case SYS_SYMLINK:
        frame->rax = sys_symlink((const char *)frame->rdi, (const char *)frame->rsi);
        break;
    case SYS_READLINK:
        frame->rax = sys_readlink((const char *)frame->rdi, (char *)frame->rsi, (u32)frame->rdx);
        break;
    case SYS_ARCH_PRCTL:
