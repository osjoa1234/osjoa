...
static volatile u32 timer_ticks = 0U;
static u32 timer_demo_complete = 0U;
...
static void handle_irq(const struct interrupt_frame *frame)
{
    ...
    if (irq == 0U) {
        ...
        console_printf("interrupt 0x%02X irq=%u tick=%u\n", frame->vector, irq, timer_ticks);

        if (timer_ticks >= INTERRUPTS_TIMER_DEMO_LIMIT && timer_demo_complete == 0U) {
            timer_demo_complete = 1U;
            pic_mask_irq(0U);
            demo_finished = 1U;
        }
    } else {
        console_set_color(0x0DU);
        console_printf("interrupt 0x%02X irq=%u\n", frame->vector, irq);
    }
    ...
    if (demo_finished != 0U) {
        ...
        console_printf("PIC timer demo complete after %u ticks\n", timer_ticks);
        qemu_debug_exit(0x00U);
    }
}
