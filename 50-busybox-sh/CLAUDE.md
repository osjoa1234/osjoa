...
`busybox sh` 프롬프트에서 `./hello`처럼 **외부 명령을 실행**시키면(=ash가 실제로 `fork()`+`execve()`를 호출하면) `49-fork-clone-fix`에서 고친 fork/clone/TLS/wait4 경로를 실제로 타게 된다. 그 기반이 없었다면 이 한 줄만으로 커널이 죽거나(page fault) 셸이 영원히 멈췄을 것이다 — 지금은 다음처럼 정상적으로(그러나 명령을 못 찾은 채) 넘어간다:
...
크래시도, 행(hang)도 없다. 다만 `Operation not permitted`(EPERM) 자체는 틀린 메시지다 — `proc_exec`의 `SYS_EXECVE` 핸들러가 실패 사유를 구분 안 하고 항상 raw `-1`을 리턴해서, glibc가 `-errno` 관례로 해석할 때 무조건 `errno=1`(EPERM)이 된다. 진짜 원인은 `initrd_open`이 `"./hello"`를 못 찾은 것(`ENOENT`가 맞는 값)이다 — `initrd_open`이 저장된 cpio 이름의 `"./"` 접두어만 벗기고 질의(query) 쪽의 `"./"`는 벗기지 않아서, `hello`는 찾아도 `./hello`는 못 찾는다. 둘 다 다음 단계 힌트로 남긴다.
...
`./hello`(위 절 참고), `hello`/`hello2`/`brk`/`mmap`/`tls`/`syscall64`/`musl_hello`/`forkclone`/`hello | pipe`/`hello | pipe | pipe`/`signal`(`Ctrl+C` 3회)도 회귀 없이 그대로 동작한다 — QEMU HMP 모니터를 유닉스 소켓으로 열고 `sendkey`를 스크립트로 순서대로 넣어, `-debugcon file:...`로 받은 로그(포트 `0xE9`는 `console.c`가 VGA 출력과 함께 항상 미러링한다)로 확인했다.
...
fork/clone/TLS/wait4 커널 수정(`boot/thread.h`/`thread.c`/`gdt.asm`/`process.c`의 `SYS_CLONE` 분기)은 `49-fork-clone-fix`에서 이미 끝났다 — 이 단계는 그 위에 busybox 관련 변경만 얹는다.

## 다음 단계 힌트

- `ls`, `cat 없는파일`, 디렉토리 관련 busybox applet은 전부 `getdents` 부재로 실패한다 — `52-vfs-ext`에서 `getdents`/`mkdir`/`unlink`를 붙일 때 같이 검증.
...
- `SYS_EXECVE`(`proc_exec`)가 실패 사유를 구분 안 하고 항상 raw `-1`을 리턴한다 — glibc는 이를 `-errno` 관례로 해석해 항상 `EPERM`(1)으로 보고한다(위 `./hello` 절의 `Operation not permitted` 참고). `initrd_open` 실패는 `ENOENT`(-2), `elf_load_process` 실패는 다른 값 등으로 구분해서 `-errno`를 리턴하도록 고치면 busybox/셸의 에러 메시지가 정확해진다.
