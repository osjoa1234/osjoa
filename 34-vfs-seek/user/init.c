...
static unsigned int sys_lseek(unsigned int fd, int offset, unsigned int whence)
{
    unsigned int pos;
    __asm__ volatile ("int $0x80" : "=a"(pos) : "0"(11U), "b"(fd), "c"(offset), "d"(whence) : "memory");
    return pos;
}
...
void _start(void)
{
    ...
    sys_lseek(fd, 7, 0);
    n = sys_read(fd, buf, 63U);
    sys_write(1U, "init: seek+read: ", 17U);
    sys_write(1U, buf, n);
