...
static void user_task(void)
{
    ...
    __asm__ volatile (
        "int $0x80"
        : "=a" (fd)
        : "a" (SYS_OPEN), "b" ("hello.txt")
    );

    __asm__ volatile (
        "int $0x80"
        : "=a" (n)
        : "a" (SYS_READ), "b" (fd), "c" (user_buf), "d" (sizeof(user_buf) - 1U)
    );
...
void kernel_main(u32 magic, u32 multiboot_info)
{
    ...
    console_printf("threads: all done -- entering user mode (ring 3), initramfs read via syscall\n");
