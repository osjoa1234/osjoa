...
- **`make run` 검증 뒤, 이번 단계가 새로 만들거나 바꾼 것이 이미 살아있는 다른 서브시스템(할당기, 인터럽트 핸들러, 스케줄러, VFS 등)의 기존 전제조건을 깨뜨리지 않는지 짚어본다.** 이건 미래에 생길 문제를 지금 미리 처리하라는 게 아니라(그건 스코프 위반), **지금 단계 자신이 건드리는 대상을 올바르게 끝냈는지**를 확인하는 것이다 — 예: 새 물리 메모리 영역을 도입하면 그 시점에 이미 존재하는 물리 할당기가 그 영역을 알게 해야 한다. 이 확인에서 걸러지는 항목이 있으면 그 단계 안에서 바로 고치고, 정말 스코프 밖이라 다음 단계로 미룰 경우엔 그 단계의 "다음 단계 힌트"에 명시적으로 남긴다.
