...
- **`make run` 검증 뒤, 이번 단계가 새로 만들거나 바꾼 것이 이미 살아있는 다른 서브시스템(할당기, 인터럽트 핸들러, 스케줄러, VFS 등)의 기존 전제조건을 깨뜨리지 않는지 짚어본다.** 이건 미래에 생길 문제를 지금 미리 처리하라는 게 아니라(그건 스코프 위반), **지금 단계 자신이 건드리는 대상을 올바르게 끝냈는지**를 확인하는 것이다 — 예: 새 물리 메모리 영역을 도입하면 그 시점에 이미 존재하는 물리 할당기가 그 영역을 알게 해야 한다. 이 확인에서 걸러지는 항목이 있으면 그 단계 안에서 바로 고치고, 정말 스코프 밖이라 다음 단계로 미룰 경우엔 그 단계의 "다음 단계 힌트"에 명시적으로 남긴다. (기능 구현 범위에 대한 규칙이며, 인터페이스 설계 방식에 대해서는 바로 아래 항목을 볼 것.)
- **새 기능/모듈은 지금 단계의 요구사항만 구현하되, 인터페이스와 자료구조는 나중에 자연스럽게 확장될 수 있는 형태로 설계한다.** 판단 기준은 "지금 이 모양을 유지한 채 나중에 요구사항이 늘어나면, 지금 만드는 함수 시그니처를 갈아엎어야 하는가?"이다 — 갈아엎어야 한다면 지금 더 열린 형태로 잡는다. 대표적으로 경계해야 할 패턴: 인스턴스가 지금 하나뿐이라는 이유로 상태를 모듈 전역(`static g_xxx`)에 두는 것 — 컨텍스트를 담는 구조체를 만들어 함수 인자로 관통시키면 나중에 인스턴스가 늘어나도(예: ext2 멀티 마운트) 호출부만 늘리면 되고 함수 시그니처는 그대로 유지된다. **이건 미래 기능을 지금 구현하라는 뜻이 아니다** — 그건 바로 위 항목이 금지하는 스코프 위반 그대로다. 오직 지금 만드는 인터페이스·자료구조의 "모양"이 나중에 다시 쓰지 않고도 확장을 흡수할 수 있는지만 미리 고려하라는 것이다.
