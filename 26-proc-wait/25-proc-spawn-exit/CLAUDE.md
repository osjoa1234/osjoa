...
### process_t : thread_t = 1:1

현재 프로세스 하나에 스레드 하나만 대응한다. 실제 OS에서 1:N을 지원하려면:
- `process_t`가 `thread_t *` 목록을 보유
- 같은 프로세스 소속 스레드끼리 페이지 디렉토리 공유
- `proc_exit` 시 소속 스레드 전체 정리
- 스레드별 독립 유저스택 필요

이 복잡성은 현 단계 목표(생명주기·주소공간 격리 이해)와 무관하므로 생략했다. 멀티스레드 프로세스를 추가한다면 28장 `fork` 이후 `SYS_CLONE` 계열 syscall을 얹는 것이 자연스러운 순서다.
