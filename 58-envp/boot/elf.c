                     ...
                     u64 phdr, u32 phnum, u32 phentsize)
{
    ...
    if (envc > USTACK_ENVP_MAX) envc = USTACK_ENVP_MAX;
    ...
    for (i = 0U; i < envc; i++) {
        ...
        sp_off -= (u64)len;
        envp_off[i] = sp_off;
        for (k = 0U; k < len; k++) top_page[sp_off + k] = (u8)envp[i][k];
    }
    ...
    frame_words[i++] = 0ULL;
    for (k = 0U; k < envc; k++) frame_words[i++] = top - 0x1000ULL + envp_off[k];
    frame_words[i++] = 0ULL;
