...
void paging_init(u64 mmap_vaddr, u32 mmap_length, u32 entry_size)
{
    u32 max_addr = 0U;
    u32 offset   = 0U;
    ...
    while (offset < mmap_length) {
        ...
        if (e->type == 1U && (e->base_addr >> 32) == 0U) {
            u32 end = (u32)e->base_addr + (u32)e->length;
...
void page_map_mmio(u64 vaddr, u32 paddr)
{
    u64 *pml4 = tbl(kernel_pml4_phys);
    u64 *pt   = walk_pt(pml4, vaddr, 0, 1);
    u32  pt_i = (u32)((vaddr >> 12) & 0x1FFULL);

    pt[pt_i] = (u64)paddr | PTE_P | PTE_RW | PTE_PCD;
    flush_tlb();
}
