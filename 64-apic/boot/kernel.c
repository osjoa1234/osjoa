...
void kernel_main(u32 magic, u32 phys_mbi)
{
...

    {
        ...
        acpi_init(rsdp_tag ? rsdp_tag->rsdp : 0);

        if (acpi_ioapic_address() == 0U) {
            console_set_color(0x0CU);
            console_printf("acpi: no MADT/IOAPIC found, cannot continue\n");
            halt_forever();
        }

        console_set_color(0x0B);
        console_printf("acpi: MADT parsed (bsp apic id=%u, ioapic id=%u base=0x%08X gsi_base=%u)\n",
                       acpi_bsp_apic_id(), acpi_ioapic_id(),
