...
static void idt_set_entry(u8 vector, u64 handler, u16 selector, u8 type_attr)
{
    idt[vector].offset_low  = (u16)(handler & 0xFFFFU);
    idt[vector].selector    = selector;
    idt[vector].ist         = 0U;
    idt[vector].type_attr   = type_attr;
    idt[vector].offset_mid  = (u16)((handler >> 16) & 0xFFFFU);
    idt[vector].offset_high = (u32)((handler >> 32) & 0xFFFFFFFFU);
...
static void handle_exception(struct interrupt_frame *frame)
{
    ...
    console_printf("interrupt 0x%02X %s err=0x%08X rip=0x%016lX\n",
                   (u32)frame->vector,
                   exception_names[frame->vector],
                   (u32)frame->error_code,
                   frame->rip);

    if (frame->vector == 14U) {
        console_printf("page fault at cr2=0x%016lX\n", read_cr2());
    }
...
{
    const u32 irq = (u32)(frame->vector - 0x20U);
...
void interrupts_init(void)
{
    ...
    for (index = 0U; index < 256U; ++index) {
        idt_set_entry((u8)index, (u64)interrupt_stub_table[index], selector, 0x8EU);
    }
...
{
    ioapic_unmask_irq(irq, (u8)(0x20U + irq));
}
