...
void phys_mem_init(u64 mmap_addr, u32 mmap_length, u32 entry_size, u32 kernel_end)
{
    u32 offset = 0U;
    u32 i;
    ...
    while (offset < mmap_length) {
        const struct mmap_entry *e =
            (const struct mmap_entry *)(mmap_addr + (u64)offset);

        if (e->type == 1U && (e->base_addr >> 32) == 0U) {
            u32 addr  = (u32)e->base_addr;
            u32 len   = (u32)e->length;
            u32 start = (addr + PAGE_SIZE - 1U) & ~(PAGE_SIZE - 1U);
            u32 end   = (addr + len) & ~(PAGE_SIZE - 1U);
