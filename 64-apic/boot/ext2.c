...
#define EXT2_DEFAULT_DMODE  0755U
#define EXT2_DEFAULT_LMODE  0777U
#define EXT2_SYMLINK_MAX_DEPTH 8U
...
static void ext2_lock_init(void)
{
    g_ext2_lock.locked = 0U;
    wq_init(&g_ext2_lock.waiters);
}
...
static int ext2_read_symlink_target(const ext2_fs_t *fs, const ext2_inode_t *inode, char *buf, u32 buf_max)
{
    static u8 blk_buf[EXT2_MAX_BLOCK_SIZE];
    ...
    if (inode->i_blocks == 0U) {
        const u8 *src = (const u8 *)inode->i_block;
        for (i = 0U; i < len; i++) buf[i] = (char)src[i];
    } else {
        u32 phys_block;

        if (ext2_resolve_block(fs, inode, 0U, &phys_block) != 0) return -1;
        if (phys_block == 0U) return -1;
        if (ext2_read_block(phys_block, fs->block_size, blk_buf) != 0) return -1;

        for (i = 0U; i < len; i++) buf[i] = (char)blk_buf[i];
    }
...
static int ext2_resolve_from(const ext2_fs_t *fs, u32 start_inode_num, const char *path, u32 *out_inode, u32 depth)
{
    ext2_inode_t cur_inode;
    u32          cur_inode_num = start_inode_num;
    const char  *p = path;

    if (depth > EXT2_SYMLINK_MAX_DEPTH) return -1;
    ...
    while (*p) {
        ...
        if (ext2_read_inode(fs, next_inode_num, &next_inode) != 0) return -1;

        if ((next_inode.i_mode & EXT2_S_IFMT) == EXT2_S_IFLNK) {
            char target[EXT2_MAX_PATH];
            u32  link_base;

            if (ext2_read_symlink_target(fs, &next_inode, target, sizeof(target)) != 0) return -1;

            link_base = (target[0] == '/') ? EXT2_ROOT_INO : cur_inode_num;

            if (ext2_resolve_from(fs, link_base, target, &next_inode_num, depth + 1U) != 0) return -1;
            if (ext2_read_inode(fs, next_inode_num, &next_inode) != 0) return -1;
        }
...
{
    return ext2_resolve_from(fs, EXT2_ROOT_INO, path, out_inode, 0U);
}
...
static void ext2_bitmap_free(const ext2_fs_t *fs, u32 bitmap_block, u32 bit_index)
{
    static u8 buf[EXT2_MAX_BLOCK_SIZE];
    u32 *words = (u32 *)buf;
    u32 w = bit_index / 32U;
    ...
    if (ext2_read_block(bitmap_block, fs->block_size, buf) != 0) return;
    words[w] &= ~(1U << b);
    ext2_write_block(bitmap_block, fs->block_size, buf);
}

static void ext2_free_block(ext2_fs_t *fs, u32 phys)
{
    u32 bit;

    if (phys == 0U) return;

    bit = phys - fs->sb.s_first_data_block;
    ext2_bitmap_free(fs, fs->gd.bg_block_bitmap, bit);
...
{
    u32 bit = inode_num - 1U;

    ext2_bitmap_free(fs, fs->gd.bg_inode_bitmap, bit);
...
static void ext2_free_indirect_block(ext2_fs_t *fs, u32 block_num)
{
    static u8 buf[EXT2_MAX_BLOCK_SIZE];
    const u32 *ptrs;
    u32 entries = fs->block_size / 4U;
    ...
    if (block_num == 0U) return;
    if (ext2_read_block(block_num, fs->block_size, buf) != 0) return;

    ptrs = (const u32 *)buf;
    for (i = 0U; i < entries; i++) {
        if (ptrs[i] != 0U) ext2_free_block(fs, ptrs[i]);
    }

    ext2_free_block(fs, block_num);
}

static void ext2_free_double_indirect(ext2_fs_t *fs, u32 block_num)
{
    static u8 buf[EXT2_MAX_BLOCK_SIZE];
    const u32 *ptrs;
    u32 entries = fs->block_size / 4U;
    ...
    ptrs = (const u32 *)buf;
    for (i = 0U; i < entries; i++) {
        if (ptrs[i] != 0U) ext2_free_indirect_block(fs, ptrs[i]);
    }

    ext2_free_block(fs, block_num);
}

static void ext2_free_triple_indirect(ext2_fs_t *fs, u32 block_num)
{
    static u8 buf[EXT2_MAX_BLOCK_SIZE];
    const u32 *ptrs;
    u32 entries = fs->block_size / 4U;
    ...
    ptrs = (const u32 *)buf;
    for (i = 0U; i < entries; i++) {
        if (ptrs[i] != 0U) ext2_free_double_indirect(fs, ptrs[i]);
    }

    ext2_free_block(fs, block_num);
}

static void ext2_free_inode_blocks(ext2_fs_t *fs, const ext2_inode_t *inode)
{
    u32 i;

    for (i = 0U; i < EXT2_DIRECT_BLOCKS; i++) {
        if (inode->i_block[i] != 0U) ext2_free_block(fs, inode->i_block[i]);
    }

    if (inode->i_block[12] != 0U) ext2_free_indirect_block(fs, inode->i_block[12]);
    if (inode->i_block[13] != 0U) ext2_free_double_indirect(fs, inode->i_block[13]);
    if (inode->i_block[14] != 0U) ext2_free_triple_indirect(fs, inode->i_block[14]);
}
...
static int ext2_mkdir_impl(const char *path)
{
    ...
    name_len = 0U;
    while (name[name_len]) name_len++;
    if (name_len == 0U || name_len > EXT2_MAX_NAME) return -1;

    if (ext2_resolve_path(fs, dir_path, &parent_inode_num) != 0) return -1;
    if (ext2_read_inode(fs, parent_inode_num, &parent_inode) != 0) return -1;
    if ((parent_inode.i_mode & EXT2_S_IFMT) != EXT2_S_IFDIR) return -1;

    if (ext2_scan_dir(fs, &parent_inode, name, &existing_inode_num) == 0) return -1;

    new_inode_num = ext2_alloc_inode(fs);
    if (new_inode_num == 0U) return -1;

    {
        u8 *raw = (u8 *)&new_inode;
        for (i = 0U; i < sizeof(new_inode); i++) raw[i] = 0U;
    }
    new_inode.i_mode        = (u16)(EXT2_S_IFDIR | EXT2_DEFAULT_DMODE);
    new_inode.i_links_count = 2U;

    if (ext2_get_or_alloc_block(fs, &new_inode, 0U, &phys_block) != 0) return -1;
    new_inode.i_size = fs->block_size;

    for (i = 0U; i < fs->block_size; i++) dir_buf[i] = 0U;

    dot_rec_len = (EXT2_DIRENT_HDR + 1U + 3U) & ~3U;

    {
        ext2_dirent_t *dot    = (ext2_dirent_t *)dir_buf;
        ext2_dirent_t *dotdot = (ext2_dirent_t *)(dir_buf + dot_rec_len);
        char *dot_name        = (char *)dot + sizeof(ext2_dirent_t);
        char *dotdot_name     = (char *)dotdot + sizeof(ext2_dirent_t);

        dot->inode     = new_inode_num;
        dot->rec_len   = (u16)dot_rec_len;
        dot->name_len  = 1U;
        dot->file_type = EXT2_FT_DIR;
        dot_name[0]    = '.';

        dotdot->inode     = parent_inode_num;
        dotdot->rec_len   = (u16)(fs->block_size - dot_rec_len);
        dotdot->name_len  = 2U;
    ...
    if (ext2_dir_insert(fs, &parent_inode, parent_inode_num, name, name_len,
                         new_inode_num, EXT2_FT_DIR) != 0) {
        return -1;
    }

    fs->gd.bg_used_dirs_count++;
    if (ext2_flush_gd(fs) != 0) return -1;
...
static int ext2_unlink_impl(const char *path)
{
    ...
    if (ext2_scan_dir(fs, &parent_inode, name, &target_inode_num) != 0) return -1;
    if (ext2_read_inode(fs, target_inode_num, &target_inode) != 0) return -1;
    if ((target_inode.i_mode & EXT2_S_IFMT) == EXT2_S_IFDIR) return -1;

    nblocks = (parent_inode.i_size + fs->block_size - 1U) / fs->block_size;
    removed = 0;

    for (block_index = 0U; block_index < nblocks && !removed; block_index++) {
        u32 phys_block;
        u32 pos = 0U;
        ext2_dirent_t *prev = 0;

        if (ext2_resolve_block(fs, &parent_inode, block_index, &phys_block) != 0) break;
        if (phys_block == 0U) continue;
        if (ext2_read_block(phys_block, fs->block_size, dir_buf) != 0) break;

        while (pos + EXT2_DIRENT_HDR <= fs->block_size) {
            ext2_dirent_t *de = (ext2_dirent_t *)(dir_buf + pos);

            if (de->rec_len == 0U) break;

            if (de->inode != 0U && de->name_len == name_len) {
                const char *ename = (const char *)de + sizeof(ext2_dirent_t);
                u32 k;

                for (k = 0U; k < de->name_len && name[k] != '\0'; k++) {
                    if (ename[k] != name[k]) break;
                }

                if (k == de->name_len && name[k] == '\0') {
                    if (prev) {
    ...
    if (target_inode.i_links_count > 0U) target_inode.i_links_count--;

    if (target_inode.i_links_count == 0U) {
        u8 *raw;
        u32 z;

        ext2_free_inode_blocks(fs, &target_inode);

        raw = (u8 *)&target_inode;
        for (z = 0U; z < sizeof(target_inode); z++) raw[z] = 0U;
        if (ext2_write_inode(fs, target_inode_num, &target_inode) != 0) return -1;
...
int ext2_unlink(const char *path)
{
    ...
    ext2_lock_acquire();
    ret = ext2_unlink_impl(path);
    ext2_lock_release();
    return ret;
}

static int ext2_symlink_impl(const char *target, const char *linkpath)
{
    ext2_fs_t *fs = &g_fs;
    char dir_path[EXT2_MAX_PATH];
    char name[EXT2_MAX_NAME + 1U];
    ...
    ext2_split_parent(linkpath, dir_path, sizeof(dir_path), name, sizeof(name));
    ...
    if (target_len == 0U || target_len >= EXT2_MAX_PATH) return -1;
    ...
    new_inode.i_mode        = (u16)(EXT2_S_IFLNK | EXT2_DEFAULT_LMODE);
    new_inode.i_links_count = 1U;
    new_inode.i_size        = target_len;

    if (target_len < sizeof(new_inode.i_block)) {
        u8 *dst = (u8 *)new_inode.i_block;
        for (i = 0U; i < target_len; i++) dst[i] = (u8)target[i];
    } else {
        static u8 blk_buf[EXT2_MAX_BLOCK_SIZE];
        u32 phys_block;
        ...
        for (i = 0U; i < fs->block_size; i++) blk_buf[i] = 0U;
        for (i = 0U; i < target_len; i++) blk_buf[i] = (u8)target[i];

        if (ext2_write_block(phys_block, fs->block_size, blk_buf) != 0) return -1;
    }

    if (ext2_write_inode(fs, new_inode_num, &new_inode) != 0) return -1;

    return ext2_dir_insert(fs, &parent_inode, parent_inode_num, name, name_len,
                            new_inode_num, EXT2_FT_SYMLINK);
}
...
int ext2_symlink(const char *target, const char *linkpath)
{
    ...
    ext2_lock_acquire();
    ret = ext2_symlink_impl(target, linkpath);
    ext2_lock_release();
    return ret;
}

static int ext2_readlink_impl(const char *linkpath, char *buf, u32 buf_max)
{
    ext2_fs_t *fs = &g_fs;
    char dir_path[EXT2_MAX_PATH];
    char name[EXT2_MAX_NAME + 1U];
    ...
    if (ext2_scan_dir(fs, &parent_inode, name, &target_inode_num) != 0) return -1;
    if (ext2_read_inode(fs, target_inode_num, &target_inode) != 0) return -1;
    if ((target_inode.i_mode & EXT2_S_IFMT) != EXT2_S_IFLNK) return -1;

    if (ext2_read_symlink_target(fs, &target_inode, buf, buf_max) != 0) return -1;

    return (int)target_inode.i_size;
}
...
int ext2_readlink(const char *linkpath, char *buf, u32 buf_max)
{
    ...
    ext2_lock_acquire();
    ret = ext2_readlink_impl(linkpath, buf, buf_max);
    ext2_lock_release();
    return ret;
}

static int ext2_lstat_impl(const char *path, u32 *out_mode, u32 *out_size)
{
    ext2_fs_t *fs = &g_fs;
    char dir_path[EXT2_MAX_PATH];
    char name[EXT2_MAX_NAME + 1U];
    ...
    *out_mode = target_inode.i_mode;
    *out_size = target_inode.i_size;
    return 0;
}
...
int ext2_lstat(const char *path, u32 *out_mode, u32 *out_size)
{
    ...
    ext2_lock_acquire();
    ret = ext2_lstat_impl(path, out_mode, out_size);
    ext2_lock_release();
    return ret;
}
...
static int ext2_open_impl(const char *path, u32 flags)
{
    ext2_fs_t *fs = &g_fs;
    u32        inode_num;
    u32        i;
    ...
    if (ext2_resolve_path(fs, path, &inode_num) != 0) {
        ...
        if ((inode.i_mode & EXT2_S_IFMT) == EXT2_S_IFREG && inode.i_size != 0U) {
            ext2_free_inode_blocks(fs, &inode);
            inode.i_size   = 0U;
            inode.i_blocks = 0U;
            for (i = 0U; i < 15U; i++) inode.i_block[i] = 0U;
...
static u32 ext2_read_impl(int bfd, u8 *buf, u32 len, u32 pos)
{
...
u32 ext2_read(int bfd, u8 *buf, u32 len, u32 pos)
{
    ...
    ext2_lock_acquire();
    ret = ext2_read_impl(bfd, buf, len, pos);
    ext2_lock_release();
    return ret;
}

static u32 ext2_write_impl(int bfd, const u8 *buf, u32 len, u32 pos)
{
    static u8     block_buf[EXT2_MAX_BLOCK_SIZE];
    ext2_fs_t    *fs = &g_fs;
    ext2_ofile_t *f;
...
u32 ext2_write(int bfd, const u8 *buf, u32 len, u32 pos)
{
    ...
    ext2_lock_acquire();
    ret = ext2_write_impl(bfd, buf, len, pos);
    ext2_lock_release();
    return ret;
}

static u32 ext2_size_impl(int bfd)
{
    ext2_fs_t *fs = &g_fs;
...
u32 ext2_size(int bfd)
{
    ...
    ext2_lock_acquire();
    ret = ext2_size_impl(bfd);
    ext2_lock_release();
    return ret;
}

static void ext2_close_impl(int bfd)
{
    ext2_fs_t *fs = &g_fs;
...
static void ext2_dup_impl(int bfd)
{
    ext2_fs_t *fs = &g_fs;
...
static u32 ext2_getdents_impl(int bfd, u8 *buf, u32 len, u32 *pos)
{
    static u8     dir_buf[EXT2_MAX_BLOCK_SIZE];
    ext2_fs_t    *fs = &g_fs;
    ext2_ofile_t *f;
...
u32 ext2_getdents(int bfd, u8 *buf, u32 len, u32 *pos)
{
    ...
    ext2_lock_acquire();
    ret = ext2_getdents_impl(bfd, buf, len, pos);
    ext2_lock_release();
    return ret;
}

static u32 ext2_mode_impl(int bfd)
{
    ext2_fs_t *fs = &g_fs;
