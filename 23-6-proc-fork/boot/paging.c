...
u32 paging_fork_dir(u32 parent_pd_phys)
{
    u32 *parent_pd  = (u32 *)parent_pd_phys;
    u32 *parent_pt0 = (u32 *)(parent_pd[0] & ~0xFFFU);
    u32  pd_phys    = page_alloc();
    u32  pt0_phys   = page_alloc();
    u32 *new_pd     = (u32 *)pd_phys;
    u32 *new_pt0    = (u32 *)pt0_phys;
    ...
    for (i = 0U; i < 1024U; i++) {
        u32 pte   = parent_pt0[i];
        u32 phys  = pte & ~0xFFFU;
        u32 ident = i * 0x1000U;

        if ((pte & 0x01U) && phys != ident) {
            u32  new_frame = page_alloc();
            u8  *src       = (u8 *)phys;
            u8  *dst       = (u8 *)new_frame;
            u32  k;
            for (k = 0U; k < 0x1000U; k++) dst[k] = src[k];
            new_pt0[i] = new_frame | (pte & 0xFFFU);
        } else {
            new_pt0[i] = pte;
        }
    }
    ...
    new_pd[0] = pt0_phys | 0x07U;
    for (i = 1U; i < 1024U; i++) new_pd[i] = parent_pd[i];
