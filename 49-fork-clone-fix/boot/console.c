...
static void console_write_unsigned64_padded(u64 value, u32 base, const char *digits, u32 width, char pad)
{
    char buffer[32];
    u32 length;
    u32 max_length;
...
static void console_vprintf(const char *format, va_list arguments)
{
    ...
    while (format[index] != '\0') {
        ...
        case 'u':
            if (is_long) {
                console_write_unsigned64_padded(va_arg(arguments, u64), 10U, "0123456789", width, pad);
            } else {
        ...
        case 'x':
            if (is_long) {
                console_write_unsigned64_padded(va_arg(arguments, u64), 16U, "0123456789abcdef", width, pad);
            } else {
