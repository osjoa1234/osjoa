...
void initrd_init(u64 start, u64 end)
{
    const u8 *p = (const u8 *)start;

    nfiles = 0U;

    while ((u64)p + 110U <= end) {
        u32 namesize;
        u32 filesize;
        const char *name;
        u32 data_off;
...
int initrd_open(const char *name)
{
    ...
    for (i = 0U; i < nfiles; i++) {
        ...
        if (n[0] == '.' && n[1] == '/') n += 2;
        if (streq(n, name)) return (int)i;
    }
    return -1;
}

u32 initrd_read(int fd, u8 *buf, u32 len, u32 pos)
{
    u32 avail;
    u32 i;
    ...
    avail = files[fd].size > pos ? files[fd].size - pos : 0U;
    if (len > avail) len = avail;

    for (i = 0U; i < len; i++) {
        buf[i] = files[fd].data[pos + i];
    }
...
{
    if (fd < 0 || (u32)fd >= nfiles) return (const u8 *)0;
...
#define INITRD_MODE_REG (0100000U | 0644U)

u32 initrd_mode(int fd)
{
    if (fd < 0 || (u32)fd >= nfiles) return 0U;
    return INITRD_MODE_REG;
}
