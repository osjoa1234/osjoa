...
static u8 *exec_load_binary(const char *path, u32 *out_size)
{
    ...
    total = 0U;
    while (total < size) {
        n = vfs_read(f, buf + total, size - total);
        if (n == 0U) break;
        total += n;
    }
    vfs_close(f);

    if (total != size) { kfree(buf); return 0; }
...
u32 proc_spawn(const char *name)
{
    ...
    if (!bin_buf) { p->state = PROC_FREE; return (u32)-1U; }

    pml4_phys = paging_clone_dir();
    entry     = elf_load_process(bin_buf, bin_size, pml4_phys,
    ...
    envp[0] = (char *)"PATH=/disk/bin:/";
    envp[1] = 0;

    p->entry      = entry;
    p->user_rsp   = elf_setup_stack(pml4_phys, argv, 1U, envp, 1U, entry, phdr, phnum, phentsize);
...
void proc_exec(const char *name, char *const argv[], char *const envp[])
{
    process_t *p = (process_t *)thread_current()->user_data;
    u8        *bin_buf;
    u32        bin_size;
    ...
    u32        argc;
    char       envp_buf[PROC_EXEC_ENVMAX][PROC_EXEC_ENVLEN];
    ...
    envc = 0U;
    while (envp && envp[envc] && envc < PROC_EXEC_ENVMAX) {
        const char *src = envp[envc];
        for (k = 0U; k < PROC_EXEC_ENVLEN - 1U && src[k]; k++) envp_buf[envc][k] = src[k];
        envp_buf[envc][k] = '\0';
        envp_copy[envc] = envp_buf[envc];
        envc++;
    }
    envp_copy[envc] = 0;
    ...
    entry = elf_load_process(bin_buf, bin_size, p->pml4_phys,
                              &brk_start, &phdr, &phnum, &phentsize);
    kfree(bin_buf);
    if (!entry) return;
    ...
    user_rsp = elf_setup_stack(p->pml4_phys, argv_copy, argc, envp_copy, envc, entry, phdr, phnum, phentsize);
