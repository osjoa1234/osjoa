...
void kernel_main(u32 magic, u32 multiboot_info)
{
    ...
    pa3 = page_alloc();
    console_set_color(0x0E);
    console_printf("page_alloc: 0x%08X 0x%08X 0x%08X\n", pa1, pa2, pa3);

    page_map_frame(0x400000U, pa1);
    p  = (volatile u32 *)0x400000U;
    *p = 0xDEADBEEFU;
    console_set_color(0x0A);
    console_printf("map virt=0x400000 -> phys=0x%08X write=0xDEADBEEF read=0x%08X %s\n",
                   pa1, *p, *p == 0xDEADBEEFU ? "OK" : "FAIL");

    page_free(pa2);
    page_free(pa3);
    console_set_color(0x07);
    console_printf("after page_free x2: %u free pages\n", phys_mem_free_count());
